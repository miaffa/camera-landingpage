"use client";

import { useState } from "react";
import { mutate } from "swr";
import { toast } from "sonner";

interface GearData {
  name: string;
  category: string;
  description: string;
  price: number;
  condition: string;
  location: string;
  images: File[];
  existingImages: string[];
  removedImages: string[];
  availability: {
    startDate: string;
    endDate: string;
    available: boolean;
  };
}

export function useGearEdit() {
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const clearError = () => setError(null);

  const editGear = async (gearId: string, gearData: GearData) => {
    setIsLoading(true);
    setError(null);

    try {
      // Create FormData for multipart upload with images
      const formData = new FormData();
      formData.append("gearId", gearId);
      formData.append("name", gearData.name);
      formData.append("category", gearData.category);
      formData.append("description", gearData.description);
      formData.append("price", gearData.price.toString());
      formData.append("condition", gearData.condition);
      formData.append("location", gearData.location);
      formData.append("availability", JSON.stringify(gearData.availability));
      formData.append("existingImages", JSON.stringify(gearData.existingImages));
      formData.append("removedImages", JSON.stringify(gearData.removedImages));
      
      // Append new image files
      gearData.images.forEach((file, index) => {
        formData.append(`image${index}`, file);
      });

      const response = await fetch("/api/gear", {
        method: "PUT",
        body: formData, // Send as FormData for multipart upload
      });

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || "Failed to update gear");
      }

      const updatedGear = await response.json();
      toast.success("Gear updated successfully!");
      
      // Revalidate gear data
      mutate("/api/gear");
      
      return updatedGear;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Failed to update gear";
      setError(errorMessage);
      toast.error(errorMessage);
      throw err;
    } finally {
      setIsLoading(false);
    }
  };

  return {
    editGear,
    isLoading,
    error,
    clearError,
  };
}
